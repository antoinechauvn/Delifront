...
export default function Home({ navigation }) {
  ...
  if(!fontsLoaded){
    ...
    const handleLogin = async () => {
      try {
          const response = await fetch('http://localhost:8080/demo/utilisateurs', {
              method: 'POST',
              headers: {
          ...
          if (response.ok) {
            console.error(email + password + nom + prenom );
              console.error(response);
              const user = await response.json();
              setUser(user);
              navigation.navigate('Home')
          } else {
            console.error('Erreur lors identification');
          }
      } catch (error) {
        ...
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center'}}>
          <View style={styles.box}>
            <Image style={styles.logo} source={require('../assets/logo.png')}/>
              ...
              <View style={styles.inputView}>
                <TextInput style={styles.inputText} onChangeText={text => setNom(text)}></TextInput>
              </View>
              <Text style={styles.inputTitre}>Prenom</Text>
              <View style={styles.inputView}>
                <TextInput style={styles.inputText} onChangeText={text => setPrenom(text)}></TextInput>
              </View>
              ...
              <Button title="S'inscrire" color="red" style={styles.buttonPress}
              onPress={() => handleLogin()}/>
              <TouchableOpacity onPress={() => navigation.navigate('Login')}>
                <Text style={{ marginTop: 10, color: 'white', textAlign: 'center' }}>
                <u>Déjà un compte ? Connectez-vous ici.</u>
                </Text>
              </TouchableOpacity>
          </View>
        </View>
